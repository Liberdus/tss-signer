...
impl LocalSignature {
    ...
    pub fn output_signature(&self, s_vec: &Vec<FE>) -> Result<Signature, TssError> {
        // 1) curve order `n` (in ECDSA literature this is often called `q`)
        let n: BigInt = FE::q(); // FE::q() should return the scalar-group order

        // 2) sum up all the partial s_i
        let mut s = s_vec.iter().fold(self.s_i.clone(), |acc, x| acc + x);

        // 3) r = R.x_coordinate mod n
        let rx = self.R.x_coor().unwrap();
        let r = ECScalar::from(&rx.mod_floor(&n));

        // 4) compute initial recovery ID from R.y
        let ry = self
            .R
            .y_coor()
            .ok_or(TssError::InvalidSig)?
            .mod_floor(&n);
        let mut recid = if ry.is_odd() { 1u8 } else { 0u8 };

        // 5) reduce s mod n, then apply “low-S” rule
        let mut s_bn = s.to_big_int().mod_floor(&n);
        // if s > n/2, replace s with (n - s) and flip recid
        let half_n = &n >> 1;
        if s_bn > half_n {
            s_bn = (&n - &s_bn).mod_floor(&n);
            recid ^= 1;
        }
        s = ECScalar::from(&s_bn);

        // 6) build the Signature and verify it one last time
        let sig = Signature { r, s, recid };
        if verify(&sig, &self.y, &self.m).is_ok() {
            Ok(sig)
        } else {
            Err(TssError::InvalidSig)
        }
    }
